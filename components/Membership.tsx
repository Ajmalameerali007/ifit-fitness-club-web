...
const PlanCard: React.FC<{ plan: Plan; onSelect: () => void; isFeatured?: boolean; }> = ({ plan, onSelect, isFeatured }) => {
    // Card styling logic remains the same
    return (
        <motion.div
        ...
        >
            ...
            <div className="mt-4">
                <p className="text-3xl font-bold text-brand-lime mb-1">{plan.price} <span className="text-lg font-normal text-gray-300">AED</span></p>
                    ...
                    >
                        {planData[selectedCategory as keyof typeof planData].map(plan => (
                            <PlanCard 
                                key={plan.name} 
                                plan={plan} 
                                onSelect={() => onSelectPlan(plan as Plan)}
